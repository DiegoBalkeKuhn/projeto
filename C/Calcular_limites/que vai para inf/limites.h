...
static double numeric_side(N*f, N*g, double a, int dir_pos){
    ...
    for(int i=0;i<5;i++){
        ...
        if(isnan(v)) continue;
        /* Se ja e infinito, conta e registra sinal */
        if(isinf(v)){
            inf_count++;
            sign_inf = (v > 0) ? 1 : -1;
            last = v;
        ...
        /* Detecta crescimento sem limite: se |v| > 1e10 e crescendo */
        if(!isnan(prev) && !isinf(prev) && fabs(v) > 1e8){
            /* Verifica se esta crescendo em relacao ao passo anterior */
            if(fabs(v) > 10.0 * fabs(prev) || fabs(prev) > 1e8){
                int s = (v > 0) ? 1 : -1;
                inf_count++;
                sign_inf = s;
                last = (double)s * INFINITY;
                continue;
            }
        }
        prev = v;
    ...
    /* Se a maioria dos pontos indica infinito, retorna infinito */
    if(inf_count >= 2 && !isnan(last) && isinf(last))
        return (double)sign_inf * INFINITY;
    return last;
}
...
static double run(const char*fs, const char*gs, double a, Side side, int vb){
    ...
    if(isinf(a)){
        ...
        if(hd){
            ...
            res = fv;
            /* [fix] Se sem denominador e valor e muito grande, retorna infinito */
            if(!hd && !isinf(res) && fabs(res) > 1e15){
                res = (res > 0) ? INFINITY : -INFINITY;
            }
        }
    ...
    /* ── Ponto finito ── */
    {
        ...
        if(!hd){
            ...
            /* fa == NAN: pode ser 0/0 interno (ex: 0^0) ou dominio invalido */
            /* Tenta avaliacao lateral para detectar divergencia */
            {
                double eps = 1e-7;
                double vp = ev(f, a + (side==ESQ ? -eps : eps));
                double vm = ev(f, a - (side==ESQ ? eps : -eps));
                if(isinf(vp) || isinf(vm)){
                    /* diverge */
                    goto DIVERGE;
                }
            }
        ...
        if(!fa_z && ga_z){
            /* num != 0, den -> 0: assintota vertical
               Se unilateral, podemos determinar o sinal pelo lado pedido */
            if(vb) printf("  num!=0, den->0 -> divergencia\n");
            if(side != BIL){
                double ep = 1e-8;
                double xside = a + (side==DIR ? ep : -ep);
                double fside = ev(f, xside);
                double gside = ev(g, xside);
                if(!isnan(fside) && !isnan(gside) && gside != 0.0){
                    double ratio = fside / gside;
                    double res = (ratio > 0) ? INFINITY : -INFINITY;
                    if(vb){ sep(); print_res(res); sep(); }
                    return res;
                }
            }
    ...
    NUM_EST:;{
        ...
        if(side==ESQ){ res=Lm; }
            ...
            if(isnan(Lm) && !isnan(Lp)){
                ...
                res=Lm;
            /* Ambos infinitos: se mesmo sinal, limite existe e e esse infinito */
            } else if(isinf(Lm) && isinf(Lp)){
                if((Lm > 0) == (Lp > 0)){
                    res = Lm; /* mesmo sinal -> limite = +-inf */
                } else {
                    if(vb) printf("  Limites laterais: L^-=%sinf  L^+=%sinf (sinais opostos)\n",
                        Lm>0?"+":"-", Lp>0?"+":"-");
                    return NAN;
                }
            } else if(isinf(Lm) && !isinf(Lp)){
                if(vb) printf("  Limites laterais divergem: L^-=%sinf  L^+=%.6g\n",Lm>0?"+":"-",Lp);
                return NAN;
            } else if(!isinf(Lm) && isinf(Lp)){
                if(vb) printf("  Limites laterais divergem: L^-=%.6g  L^+=%sinf\n",Lm,Lp>0?"+":"-");
                return NAN;
            } else {
    ...
    DIVERGE:;{
        ...
        /* Determina sinal de cada lado (considera valores muito grandes como inf) */
        int lp_big = isinf(lp) || fabs(lp) > 1e8;
        int lm_big = isinf(lm) || fabs(lm) > 1e8;
        int lp_pos = (lp > 0);
        int lm_pos = (lm > 0);

        /* Limite unilateral: usa apenas o lado pedido */
        if(side == ESQ){
            if(!isnan(lm) && lm_big){
                res = lm_pos ? INFINITY : -INFINITY;
                if(vb) printf("  Limite esquerdo -> %sinf\n", res>0?"+":"-");
            } else { res = NAN; }
            if(vb){ sep(); print_res(res); sep(); }
            return res;
        }
        if(side == DIR){
            if(!isnan(lp) && lp_big){
                res = lp_pos ? INFINITY : -INFINITY;
                if(vb) printf("  Limite direito -> %sinf\n", res>0?"+":"-");
            } else { res = NAN; }
            if(vb){ sep(); print_res(res); sep(); }
            return res;
        }
        ...
        /* Bilateral */
        if(!isnan(lm) && !isnan(lp) && (lp_big || lm_big)){
            if(lp_pos == lm_pos){
                res = lp_pos ? INFINITY : -INFINITY;
                if(vb) printf("  Mesmo sinal -> diverge %sinf\n", res>0?"+":"-");
            } else {
                /* Sinais opostos: limite bilateral nao existe, mas laterais sao +-inf */
                if(vb){
                    printf("  Limite bilateral NAO EXISTE (assintota vertical):\n");
                    printf("    L^- = %sinfinito\n", lm_pos?"+":"-");
                    printf("    L^+ = %sinfinito\n", lp_pos?"+":"-");
                }
                sep();
                printf("  RESULTADO: Assintota vertical em x=%.6g\n", a);
                printf("    Limite pela esquerda: %sinfinito\n", lm_pos?"+":"-");
                printf("    Limite pela direita : %sinfinito\n", lp_pos?"+":"-");
                printf("    Limite bilateral    : NAO EXISTE\n");
                sep();
                return NAN;
            }
        } else if(!isnan(lm) && !isnan(lp) && lp*lm > 0){
